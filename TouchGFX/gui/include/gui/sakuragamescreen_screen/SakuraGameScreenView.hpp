...
public:
    ...
    void updateBossHPDisplay();        // NEW: Boss HP display
    void hideAllPlayerHPWidgets();
    void hideAllBossHPWidgets();       // NEW: Boss HP widgets

    // ===== CHAKRA SYSTEM =====
    void updatePlayerChakraDisplay();
    void updateAIChakraDisplay();      // Boss still uses chakra
    ...
    void givePlayerPowerUp();          // NEW: Only player gets power-ups
    void applyInstantPowerUp(bool isPlayer, int type);
    void updatePlayerPowerUpDisplay();
    void updateAIPowerUpDisplay();     // Keep for compatibility (empty function)
    void hideAllPlayerPowerUps();
    void hideAllAIPowerUps();          // Keep for compatibility
...
protected:
    ...
    bool easterEggActivated;               // Flag cho Easter Egg
    int easterEggTimer;                   // Timer cho Easter Egg (3 giây)
    static const int EASTER_EGG_WIN_TIME = 3000;  // 3 giây rồi chuyển WIN_SCENE ngay
};

#endif // SAKURAGAMESCREEN_VIEW_HPP
